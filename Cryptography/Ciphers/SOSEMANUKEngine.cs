...
namespace ObscurCore.Cryptography.Ciphers
{
    public sealed class SOSEMANUKEngine : IStreamCipher
    {
        ...
        public void ProcessBytes (byte[] inBytes, int inOff, int len, byte[] outBytes, int outOff) {
            if (!_initialised) throw new InvalidOperationException(AlgorithmName + " not initialised.");
            if ((inOff + len) > inBytes.Length) throw new DataLengthException("Input buffer too short.");
            if ((outOff + len) > outBytes.Length) throw new DataLengthException("Output buffer too short.");
            //CheckLimitExceeded();
        ...
        public void GetRawKeystream(byte[] buffer, int offset, int len) {
            if (!_initialised) throw new InvalidOperationException(AlgorithmName + " not initialised.");
            if ((offset + len) > buffer.Length) throw new DataLengthException("Output buffer too short.");
            //CheckLimitExceeded();
