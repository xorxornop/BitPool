...
namespace BitPool.Cryptography.KDF.CryptSharp
{
	...
	/*
	 * 		Modified for use in ObscurCore (removed fixed output size restriction) on 07-02-2013 (dd-mm-yyyy)
	 */
    ...
    {
		// modified (commented out to disable)
        ...
        {
            using (Pbkdf2 kdf = GetStream(key, salt, cost, blockSize, parallel, maxThreads, output.Length)) {
                kdf.Read (output);
            }
        }

        public static byte[] GetEffectivePbkdf2Salt (byte[] key, byte[] salt,
		                                             int cost, int blockSize, int parallel, int? maxThreads, int HLen)
        {
            Helper.CheckNull ("key", key);
            Helper.CheckNull ("salt", salt);
            return MFcrypt (key, salt, cost, blockSize, parallel, maxThreads, HLen);
        }
        ...
        public static Pbkdf2 GetStream (byte[] key, byte[] salt,
                                       int cost, int blockSize, int parallel, int? maxThreads, int HLen)
        {
            byte[] B = GetEffectivePbkdf2Salt (key, salt, cost, blockSize, parallel, maxThreads, HLen);
